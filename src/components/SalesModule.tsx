      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-violet-200 p-5 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <label className="text-xs font-semibold text-slate-700">Vendedor<select value={vendedorId} onChange={(e) => { const id = e.target.value; setVendedorId(id); const seller = sellers.find((item) => item.id === id); if (seller?.porcentajeComision !== undefined) setPorcentaje(String(seller.porcentajeComision)); }} className="mt-1 w-full p-2 border rounded-lg font-normal"><option value="">Seleccione</option>{sellers.map((seller) => <option key={seller.id} value={seller.id}>{seller.primerNombre} {seller.primerApellido}</option>)}</select></label>
